...
Escena::Escena()
{
	...
	copo = new CopoNieve("./img/copo_trozo.jpg");
	...
	if (!obj_textura) {
	    ...
	    if (luz_activa) {
	 	   switch(material_actual) {
	 		   case 0:
	    			glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, colorAmbienteDifusoBlanco);
	    			glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, colorEspecularBlanco);
	    			glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, brilloBlanco);
					luz->activarLuz(0);
			 		luz->activarTrasladarLuz(traslacion_luz2);
	    			break;
				case 1:
					glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, colorAmbinteDifusoTurquesa);
					glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, colorEspecularTurquesa);
					glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, brilloTurquesa);
					luz->activarLuzBlanca();
					break;
	 	   }
	    }
	} else {
...
bool Escena::teclaPulsada( unsigned char tecla, int x, int y )
{
   ...
   switch( toupper(tecla) )
   {
	  ...
	  case 'H' :
	  	traslacion_luz2 -= 1.0;
		//cout << "traslacion luz: " << traslacion_luz2 << endl;
		break;
	  case 'J' :
  	  	traslacion_luz2 += 1.0;
  		//cout << "traslacion luz: " << traslacion_luz2 << endl;
	  ...
	  case 'E' :
		...
		break;
	  case '1' :
	  	colorAmbinteDifusoTurquesa = {0.9, 0.7, 0.5, 1.0};
		break;
	   case '2' :
	   	colorEspecularTurquesa = {0.2, 0.2, 0.2, 1.0};
		break;
	   case '3' :
	   	brilloTurquesa = 20.0;
		break;
