...
void ObjPLY::calcularCajaFrontera() {
	...
	cara_caja_positiva = {(x_max + x_min) / 2, (y_max + y_min) / 2, z_max};
	cara_caja_negativa = {(x_max + x_min) / 2, (y_max + y_min) / 2, z_min};
}

void ObjPLY::calcularNormalesVertice() {
	Tupla3f calculo;
	normalesVertices.resize(vertices.size());

	for (int i = 0; i < vertices.size(); i++) {
		if (vertices[i](Z) > 0)
			calculo = vertices[i] - cara_caja_negativa;
		else
			calculo = vertices[i] - cara_caja_positiva;
		normalesVertices[i] = calculo;
	}
}

void ObjPLY::calcular_normales() {
	Tupla3f v1, v2, prod_vect;
	calcularCajaFrontera();
	calcularNormalesVertice();
