...
bool Escena::teclaPulsada( unsigned char tecla, int x, int y )
{
   ...
   switch( toupper(tecla) )
   {
      ...
      case 'C' :
        ...
        break;
	  case 'V' :
	  	camara_perspectiva = !camara_perspectiva;
		cout << "cambiada perspectiva" << endl;
		change_observer();
		break;
   }
...
void Escena::change_observer()
{
  ...
  glLoadIdentity();
  camaras[camara_actual].setObservador(camara_perspectiva);
}

void Escena::ratonMovido(int x, int y)
{
    if (!camaras[camara_actual].esCamaraOrto() && !camara_perspectiva)
    {
